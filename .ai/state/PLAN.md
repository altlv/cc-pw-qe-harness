...
| 7   | **E6 — enforce the session report** through `check-report`                                                                          | The skill asks for observations / questions / defects / not-reached, and nothing checks it                                                                                                         |
| 8   | **E7 — a real agent session**                                                                                                       | The empirical test of whether any of the above is enough. Needs E5 and a key                                                                                                                       |

### Smaller, unblocked

| #   | Item                                                                                                                                           |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 9   | **Analyzer blind to Page Objects** — `navigation-only` fires on PO-based tests because the interaction reads `exam.clickNext()`, not `.click(` |
| 10  | **LICENSE** — a public repo with none is legally unusable. **User's decision**                                                                 |
| 11  | CI actions target Node 20; GitHub is migrating to 24. A `@v5` bump clears the annotation                                                       |
| 12  | `apps/todo-fixture/coverage.md` — three skills point at `apps/<app>/coverage.md` and only countdown-timer has one                              |
| 13  | Fresh-clone verification: `npm ci` → browsers → all suites, proving it works for someone who is not us                                         |
| 14  | The crawl writes no artefact, so two crawls cannot be diffed                                                                                   |
| 15  | Scans overwrite and nothing reads one back — no drift detection between runs                                                                   |
| 16  | Recipes 0 of 9 · agent contracts 7 of 22 · schemas for requirements-analysis and triage-report not ported                                      |
