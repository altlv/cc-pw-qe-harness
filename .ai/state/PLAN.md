...
| 2   | **A way to see.** No role has a screenshot tool, and `visual-inspection` says outright that it cannot run without one                                                                                                                                                                                                    | —          | Roughly half the defects found by hand were visible and unqueryable: a tile 30px short, a panel over the footer, a cart preview off the page edge. Also gates any visual work the driver does |
| 3   | **Agent-to-skill pairing.** `honesty-check` and `work-discipline` are read by no role; `bug-report` never reaches the exploratory tester; `risk-assessment` reaches nobody who runs `test-design`. Cross-cutting skills into the shared guardrails, then one deliberate pass, then a test asserting no skill is orphaned | —          | Cheap, and a skill nothing reads is a skill that does not exist. A mutation checks a role cannot name a missing skill; nothing checks the inverse                                             |

### Then — in this order

| #   | Item                                                                                                                                | Blocked by    | Why                                                                                                                                                                             |
| --- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4   | **E5 — the driver.** Action selection under E4, obeying the environment policy, diffing via `identity.ts`                           | 1, 2          | The blocker on the benchmark number. Everything the harness misses, it misses for want of a click                                                                               |
| 5   | **The map is a single state.** Transient surfaces — mini-cart, dropdown, modal, toast, drawer — appear in no inventory and no crawl | 4             | Reaching them means opening them, which means interacting. A defect was missed for exactly this reason: the cart page was tested thoroughly and the cart _preview_ never opened |
| 6   | **E6 — enforce the session report** through `check-report`                                                                          | —             | Could start now; wanted before E7, because an unenforced report format is how a session's findings quietly go missing                                                           |
| 7   | **E7 — a real agent session against the scored benchmark**                                                                          | 1–4, 6, a key | The empirical test of all of it, and the only thing that turns "better" from opinion into a number                                                                              |

### Independent — any time, in any order

| #   | Item                                                                           | Why                                                                                                                                                                     |
| --- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8   | **`storageState`**, captured once by a person and replayed by the harness      | The authenticated half of every app is otherwise permanently dark. An agent never enters credentials, so this is the only route in: `playwright codegen --save-storage` |
| 9   | **A performance pass** — `performance.getEntriesByType`, roughly fifteen lines | One of the five benchmark categories has no capability at all. Fifteen hand-written lines found ~3x oversized images, 2.2MB of payload and a dead CDN                   |
